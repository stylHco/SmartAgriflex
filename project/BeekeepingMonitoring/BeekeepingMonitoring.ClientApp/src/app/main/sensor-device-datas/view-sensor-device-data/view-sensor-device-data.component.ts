...
import {SensorDeviceDatasClient, SensorDeviceDataDetailsModel} from "../../../@core/app-api";
import {
  SensorDeviceDataRepresentingService,
} from "../../../@core/sensor-device-datas/sensor-device-data-representing.utils";
import {SensorDeviceRepresentingComponent} from "../../../@core/sensor-devices/sensor-device-representing.utils";
...
@UntilDestroy()
@Component({
  templateUrl: './view-sensor-device-data.component.html',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
    ...
    SensorDeviceRepresentingComponent,
    NullReplacerModule,
    DateTimeDisplayModule,
  ],
})
export class ViewSensorDeviceDataComponent implements OnInit {
  item!: SensorDeviceDataDetailsModel;
  ...
  constructor(
    ...
    protected readonly representingService: SensorDeviceDataRepresentingService,
  ) {
  }
  ...
  ngOnInit(): void {
    this.item = (this.activatedRoute.snapshot.data['item'] as ApiResult<SensorDeviceDataDetailsModel>).value!;
  ...
  delete() {
    this.sensorDeviceDatasClient.delete(this.item.id)
      .pipe(
...
@Injectable()
export class ViewSensorDeviceDataTitleProvider extends ResolvedApiItemTitleProvider<SensorDeviceDataDetailsModel> {
  private readonly representingService = inject(SensorDeviceDataRepresentingService);

  getItemTitle(item: SensorDeviceDataDetailsModel): Changeable<string> {
